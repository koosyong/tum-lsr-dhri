...
PCTracking::PCTracking(ros::Publisher* _pub_scene, ros::Publisher* _pub_model, FuncPub _funcPub, int dimension, double cont_sampling, double cont_simplify, double segTol)
{
    pub_scene = _pub_scene;
    pub_model = _pub_model;
    funcPub = _funcPub;
    tracks = new PCTrackContainer(10);
    imft = new IMFT<PCObject>(2, 60, 100, &weight_l2_rev, &weight_l2_rev);
...
void PCTracking::run(CloudPtr pCloud)
{
    ...
    if(tracks->numTracks() < 1 && pCloud->points.size() <= segmentation_minSize) return;
    else if(tracks->numTracks() < 1 && pCloud->points.size() > segmentation_minSize){
        segmentationGMM(pCloud, objects);
    }
    // tracking begins
    else if(tracks->numTracks() > 0){
        ...
        int nObjects = predictiveObjects.objects.size();
         ROS_INFO("# of predictiveObjects: %d", nObjects);
...
void PCTracking::segmentationGMM(CloudPtr pCloud, PCObjectContainer& objects)
{
    ...
    for(int i=0;i<objects.numObjects();i++){
        objects.objects.at(i)->gaussianTrackingInit(2, 2, 1000, &weight_gaussian_predictive_rev, &weight_gaussian_predictive_rev);
...
void PCTracking::pointMatching(PCObjectContainer& predictiveObjects, CloudPtr pCloud, CloudPtr unmatchedPoints, vector<CloudPtr>& observedPointsList)
{
    ...
    for(int i=0;i<pCloud->points.size();i++){
        Point point(pCloud->points.at(i), dim);
...
void PCTracking::trackingGaussians(PCObject* predictiveObject, CloudPtr observedPoints, PCObjectContainer& updatedObjects)
{
    ...
    for(int i=0;i<observedPoints->points.size();i++){
        Point point(observedPoints->points.at(i), dim);
    ...
    if(num != 0){
        ...
        for(int i=0;i<newObjects.size();i++){
            if(newObjects.at(i).gmm.size()>1){
                newObjects.at(i).gaussianTrackingInit(2, 2, 1000, &weight_gaussian_predictive_rev, &weight_gaussian_predictive_rev);
