...
PCTracking::PCTracking(ros::Publisher* _pub_scene, ros::Publisher* _pub_model, FuncPub _funcPub)
{
    ...
    funcPub = _funcPub;
    tracks.reset(new PCTrackContainer(10));
    imft.reset(new IMFT<PCObject>(2, 25, 100, &weight_l2, &weight_l2));
    ...
    maxProbAssociation = 0.00000;

    thrProbScene = 0.000001;
    ...
    // tracking begins
      ...
      for(int i=0;i<oldObjectsNonupdated.objects.size();i++)
        ...
        for(int i=0;i<nObjects;i++){
            ...
            shared_ptr<PCObject> predictiveObject = predictiveObjects.objects.at(i);
...
//    shared_ptr< TrackContainer<PCObject> > trackPtr;
    tracks = imft->extractTracks();
//    *tracks = (PCTrackContainer)(*trackPtr);
//    tracks = (shared_ptr< TrackContainer<PCObject> >)(imft->extractTracks());
    cnt ++;
...
//    typename pcl::search::KdTree<PointT>::Ptr tree (new pcl::search::KdTree<PointT>);
    ...
    for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin (); it != cluster_indices.end (); ++it)
    {
        ...
        for (std::vector<int>::const_iterator pit = it->indices.begin (); pit != it->indices.end (); pit++){
            Point point;
            point.pos[0] = pCloud->points[*pit].x;
            point.pos[1] = pCloud->points[*pit].y;
            point.pos[2] = pCloud->points[*pit].z;
            point.rgb[0] = pCloud->points[*pit].r;
            point.rgb[1] = pCloud->points[*pit].g;
            point.rgb[2] = pCloud->points[*pit].b;
            object.insert(point);
        }
        object.initialGMM(scale, percent);
        if(object.gmm.size()>1)
...
void PCTracking::pointMatching(PCObjectContainer& predictiveObjects, CloudPtr pCloud, CloudPtr unmatchedPoints, vector<CloudPtr>& observedPointsList)
{
    ...
    for(int i=0;i<pCloud->points.size();i++){
        Point point;
        point.pos[0] = pCloud->points.at(i).x;
        point.pos[1] = pCloud->points.at(i).y;
        point.pos[2] = pCloud->points.at(i).z;
        point.rgb[0] = pCloud->points.at(i).r;
        point.rgb[1] = pCloud->points.at(i).g;
        point.rgb[2] = pCloud->points.at(i).b;

        // check the probability of the point to each object
        int maxTrack = 0;
        double maxProb = 0.;
        for(int j=0;j<nObjects;j++){
            shared_ptr<PCObject> object = predictiveObjects.objects.at(j);//tracks->tracks.at(j)->lastFrame().object;
...
void PCTracking::registrationGMMs(PCObjectContainer& oldObjects, CloudPtr pCloud, PCObjectContainer& registeredObjects)
{
    ...
    vector< shared_ptr<PCObject> > scenes;
    scenes.resize(nObjects);

    for(int i=0;i<nObjects;i++){
        scenes.at(i).reset(new PCObject);
    }
//    scenes.reset(new PCObject[nObjects]);
    ...
    for(int i=0;i<nObjects;i++){
        if(scenes.at(i)->points.size() > segmentation_minSize){
            scenes.at(i)->initialGMM(scale, percent);
            ...
            PCGMMReg gmmreg;
            gmmreg.init(oldObjects.objects.at(i), scenes.at(i), 100, funcPub);
            gmmreg.run();
...
void PCTracking::trackingGaussians(shared_ptr<PCObject> predictiveObject, CloudPtr observedPoints, PCObjectContainer& updatedObjects)
{
    // without filtering
    ...
    if(num != 0){
        ...
        for(int i=0;i<newObjects.size();i++){
            if(newObjects.at(i).gmm.size()>1){
                newObjects.at(i).gaussianTrackingInit(2, 3, 1000, &weight_gaussian_predictive, &weight_gaussian_predictive);
