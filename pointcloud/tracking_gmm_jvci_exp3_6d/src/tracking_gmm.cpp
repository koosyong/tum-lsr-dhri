...
{
    static int cnt = 0;
    ROS_INFO("I heard RGB at frame : %d", cnt);
...
*/
    ...
    ROS_INFO("Computation time [FPS] : %f", (float)cnt / (float)computationT);
...
int main (int argc, char** argv)
{
    ...
    pctracking = new PCTracking(&pub_scene, &pub_model, &publishPointCloud, cont_sampling, cont_simplify);
    ros::spin();
