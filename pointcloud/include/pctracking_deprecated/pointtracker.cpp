...
PointTracker<Ptr>::PointTracker(int nWindow, bool isDebug) : Parent(nWindow, isDebug)
{
    m_isDebug = isDebug;
}
...
template<typename Ptr>
bool PointTracker<Ptr>::twoGroupMatching(CloudPtr &pGroupA, pcl::PointXYZ velA, CloudPtr &pGroupB, pcl::PointXYZ velB, CloudPtr &fGroup, Cloud &tGroupA, Cloud &tGroupB)
{
    // make frame 1
...
template<typename Ptr>
double PointTracker<Ptr>::twoGroupMatchingScore(CloudPtr &pGroupA,  pcl::PointXYZ velA, CloudPtr &pGroupB)
{
    // make frame 1
    VecPtr frame1, frame2;
    for(int i=0;i<pGroupA->size();i++){
...
template<typename Ptr>
void PointTracker<Ptr>::confirmDGraph()
{
...
template<typename Ptr>
double PointTracker<Ptr>::gain(typename Parent::V pNode, typename Parent::V cNode, double alpha)
{
