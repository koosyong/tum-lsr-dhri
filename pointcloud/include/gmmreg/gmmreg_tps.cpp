...
void gmmreg_tps::tpsInit(int _level, char* _sigma, char* _max_function_evals, char* _lamda, char* _fix_affine)
{
    // set init param
    set_init_params();
    // prepare_own_options
    prepare_own_options(_level, _sigma, _max_function_evals, _lamda, _fix_affine);
...
int gmmreg_tps::set_init_params() {
...
void gmmreg_tps::perform_transform(const vnl_vector<double> &x) {
  ...
  if (b_normalize) {
    denormalize(output_model,scene_centroid, model_scale);
//    denormalize(model,scene_centroid, model_scale);
//    denormalize(scene,scene_centroid, scene_scale);
  }
//  std::cout<<"param"<<std::endl<<param_affine;
...
void gmmreg_tps::prepare_own_options(int level, char* sigma, char* max_function_evals, char* lamda, char* fix_affine)
{
    multi_scale_options(level, sigma, max_function_evals);
    char delims[] = " -,;";
    char s_lambda[256]={0};
    strcpy(s_lambda, lamda);
...
void gmmreg_tps::set_affine_and_tps(const vnl_vector<double>& x) {
  ...
  int rows_x = x.size()/d;
  if (func->fix_affine) {   // affine is given,  x does not include affineparam_all
