...
void gmmreg_base::init(vnl_matrix<double>& _model, vnl_matrix<double>& _scene, vnl_matrix<double>& _ctrl_pts, bool _normalize)
{
    // prepare_input
    model_scale = 1;
    scene_scale = 1;
    ...
    prepare_common_options(_normalize);

}

void gmmreg_base::run()
{
    // start
    ...
    if (b_normalize) {
      ...
      denormalize(model,model_centroid, model_scale);
...
void gmmreg_base::prepare_common_options(bool _normalize)
{
    if(_normalize) b_normalize = 1;
    else b_normalize = 0;
    if (b_normalize) {
      normalize(model,model_centroid,model_scale);
      scene_centroid = model_centroid;
//      scene_scale = model_scale;
      normalize(scene,scene_centroid);
//      normalize(ctrl_pts,model_centroid,model_scale);
      std::cout<<"model_scale:"<<model_scale<<std::endl;
       std::cout<<"scene_scale:"<<scene_scale<<std::endl;
    }
}
...
void gmmreg_base::multi_scale_options(int _level, char* _sigma, char* _max_function_evals)
{
    level = _level;
    char s_scale[256]={0}, s_func_evals[256] ={0};
    char delims[] = " -,;";
    strcpy(s_scale, _sigma);
    parse_tokens(s_scale, delims,v_scale);
    if (v_scale.size()<level) {
      std::cerr<< " too many levels " << std::endl;
      exit(1);
    }
    strcpy(s_func_evals, _max_function_evals);
