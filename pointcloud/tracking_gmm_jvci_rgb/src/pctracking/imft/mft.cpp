...
MFT<Ptr>::MFT(int nWindow, bool isDebug)
{
    m_isDebug = isDebug;
    m_nWindow = nWindow;
    m_cntTrack = 0;
...
template<typename Ptr>
void MFT<Ptr>::setFrame(VecPtr ptrs)
{
    cnt++;
    if(m_isDebug)
...
template<typename Ptr>
void MFT<Ptr>::setCurrentFrame(int n)
{
...
template<typename Ptr>
void MFT<Ptr>::addToDGraph(VecPtr ptrs)
{
    // save old edges
    m_vecOldEdge.clear();
...
/*
template<typename Ptr>
double MFT<Ptr>::gain(V pNode, V cNode)
{
    double weight;
    double dst;
    Track track;
...
template<typename Ptr>
void MFT<Ptr>::twoFrameCorresponding(vector<ListGraph::Node> vecUFrame, vector<ListGraph::Node> vecVFrame)
{
...
template<typename Ptr>
bool MFT<Ptr>::eraseNodeinTrack(int nTrack, int nodeId)
{
    for(int i=0;i<m_tracks.size();i++){
...
template<typename Ptr>
bool MFT<Ptr>::deleteTrack(int num)
{
    vector<ListGraph::Node> vecDelNode;
    for(ListGraph::NodeIt n(m_g); n != INVALID; ++n){
...
template<typename Ptr>
std::vector<Ptr> MFT<Ptr>::getUnmatchedPtrs()
{
...
template<typename Ptr>
void MFT<Ptr>::deleteNodeInLastFrame(Ptr ptr)
{
    vector<ListGraph::Node> vecDelNode;
    for(ListGraph::NodeIt n(m_g); n != INVALID; ++n){
        if((*m_gNodeMap)[n].nFrame == cnt && !(*m_gNodeMap)[n].isIn){
...
template<typename Ptr>
void MFT<Ptr>::updatePtrsInLastFrame(VecPtr ptrs)
{
    for(ListGraph::NodeIt n(m_g); n != INVALID; ++n){
