...
//                        cout<<"Label"<<labels<<endl;
            ...
            for(int i=0;i<k;i++){
                ...
                Eigen::MatrixXd cov_set = Eigen::MatrixXd::Zero(6,6);
                for(int j = 0;j<num;j++){
            ...
            for(int i = 0;i<k;i++){
                ...
                for (int j = 0;j<n;j++){
                    ...
                    if(method == SIMPLE_HCL2){
                        ...
                        energy1 = 1./sqrt(pow(2*pi,6)*(covi+covi).determinant());
                        energy2 = 1./sqrt(pow(2*pi,6)*(covi+covj).determinant())*exp(-0.5*a);
                        energy3 = 1./sqrt(pow(2*pi,6)*(covj+covj).determinant());
...
double PCObject::evalGMM(Point x)
{
    ...
    for(int i=0;i<gmm.size();i++){
        ...
        eval += weight * 1./sqrt(pow(2*pi,6)*det) * exp(-0.5*a);
//        cout<<"pos "<<pos<<endl;
//        cout<<"mean "<<mean<<endl;
...
void PCObject::makeTopology()
{
    ...
    for(ListGraph::NodeIt u(*topology_graph); u != INVALID; ++u){
        ...
        for(v; v != INVALID; ++v){
            ...
            if(weight > 0.77){ // lower is stronger
                ListGraph::Edge edge = topology_graph->addEdge(u,v);
                (*topology_edgeMap)[edge] = weight;

                Edge e;
                e.u.pos[0] = (*topology_nodeMap)[u].mean[0];
                e.u.pos[1] = (*topology_nodeMap)[u].mean[1];
                e.u.pos[2] = (*topology_nodeMap)[u].mean[2];
                e.u.rgb[0] = (*topology_nodeMap)[u].mean[3];
                e.u.rgb[1] = (*topology_nodeMap)[u].mean[4];
                e.u.rgb[2] = (*topology_nodeMap)[u].mean[5];

                e.v.pos[0] = (*topology_nodeMap)[v].mean[0];
                e.v.pos[1] = (*topology_nodeMap)[v].mean[1];
                e.v.pos[2] = (*topology_nodeMap)[v].mean[2];
                e.v.rgb[0] = (*topology_nodeMap)[v].mean[3];
                e.v.rgb[1] = (*topology_nodeMap)[v].mean[4];
                e.v.rgb[2] = (*topology_nodeMap)[v].mean[5];
                e.weight = weight;
                edges.push_back(e);
            }
        }
